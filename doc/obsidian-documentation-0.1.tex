...
\verb|cmake -DPREFIX=/usr/local/obsidian . |\\

Also note that you can of course change the directory where obsidian will be installed, by modifying the PREFIX variable in the command above.

\subsection{Let's roll, baby !}
...
Ok now that the compilation process is ok, let's install obsidian on the Système, according to the PREFIX variable declared earlier.To install Obsidian, Nothing is more simple. From the Obsidian project Root Directory, just type the following:\\
